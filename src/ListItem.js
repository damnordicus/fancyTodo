...
function ListItem({id, index, title, creator_id, is_complete, comments, setCurrentItem, name, fetchData}) {
    ...
    useEffect(() => {
        ...
        const getUsername = async () => {
            const getName = await fetch(`http://localhost:8080/users/`)
            const data = await getName.json();
            for(let i = 0; i < data.length; i++){
                if(data[i].id === creator_id){
    ...
    return (
      <div key={index} id="item">
        <div
          id="title"
          onClick={editTitle}
          style={{ textDecoration: localComplete ? "line-through" : "none" }}
          onMouseOver={() => viewComments}
        >
          {showEdit ? (
            <EditItem
              id={id}
        ...
        <button id="deleteButton" name="delete" onClick={deleteTask}>
          <img src={process.env.PUBLIC_URL + "./trash.svg"} alt="trash" width="15" />
        </button>
        <p>{creator_id ? name : null}</p>
      </div>
    );
}
